...
DECODED_BASE=$(bitcoin-cli decoderawtransaction "$BASE_TX")

TXID=$(echo "$DECODED_BASE" | jq -r '.txid')

FIRST_OUTPUT_VALUE=$(echo "$DECODED_BASE" | jq '(.vout[0].value * 100000000) | round')
...
UTXO_VALUE=$(echo "$DECODED_BASE" | jq ".vout[$UTXO_VOUT_INDEX].value * 100000000 | round")
...
# WRITE YOUR SOLUTION BELOW:
TX_SIZE=$(echo "$DECODED_BASE" | jq -r "(10 + ($NUM_INPUTS * 68) + ($NUM_OUTPUTS * 31))")
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX= $(bitcoin-cli decoderawtransaction "$RAW_TX" | jq -r \
  --arg addr "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address==$addr) | .n')
...
CHILD_TX_SIZE= $(echo "$CHILD_INPUTS" | jq -r 'length * 180 + 34 * 1') # Approximate size in bytes
...
# WRITE YOUR SOLUTION BELOW:
TIMELOCK_INPUTS= '[{"txid":"'$SECONDARY_TXID'","vout":0,"sequence":4294967293}]'  # sequence < 0xffffffff-1 for RBF and CSV
